...
[ -n "${beep_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${beep_ORIG_GROUP}.app/group.${beep_NEW_GROUP}.app/g" "ios/beep Share/beep Share.entitlements"
	sed -i "" "s/group.${beep_ORIG_GROUP}.app/group.${beep_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${beep_ORIG_GROUP}.app/group.${beep_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${beep_ORIG_GROUP}.app/${beep_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${beep_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${beep_ORIG_TEAM}/${beep_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for beep.app and beep.app.beep-Share)
...
[ -n "${beep_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/beep.ipa"
  rm -rf "${TMPDIR}"
}
