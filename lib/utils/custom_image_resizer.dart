...
Future<MatrixImageFileResizedResponse?> customImageResizer(
  ...
  try {
    ...
    image = img.decodeImage(arguments.bytes);
    if (image == null) {
      throw Exception('Failed to decode image.');
    }
  } catch (e, s) {
    Logs().e("Could not decode image", e, s);
    return null;
  }
  ...
  if (width > max || height > max) {
    ...
    // Resize the image
    final resizedImage = img.copyResize(image, width: w, height: h);
    image = resizedImage;
  }

  final jpegBytes = Uint8List.fromList(img.encodeJpg(image, quality: 75));

  return MatrixImageFileResizedResponse(
    bytes: jpegBytes,
    width: image.width,
    height: image.height,
    blurhash: arguments.calcBlurhash ? encodeBlurhash(image) : null,
  );
}

String encodeBlurhash(img.Image image) {
  // You would need to implement a blurhash encoder or use an existing one.
  // This is a placeholder function.
  return 'placeholder_blurhash';
}
